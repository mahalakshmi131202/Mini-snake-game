...
function showPoints(x, y, text = "+10") {
    const pointsText = new PIXI.Text(text, {
    ...
    pointsText.x = x * box + box / 2;
    pointsText.y = y * box + box / 2;

    uiLayer.addChild(pointsText);
    ...
    ticker.add(() => {
        pointsText.y -= 1;
        pointsText.alpha -= 1 / life;
        life--;

        if (life <= 0) {
            uiLayer.removeChild(pointsText);
            ticker.stop();
        }
    });
...
function update() {
    ...
    // Show +10 on board at fruit position
    showPoints(food.x, food.y, "+10");
